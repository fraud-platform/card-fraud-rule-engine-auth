...
- Project: `card-fraud-rule-engine-auth`
...
Observed results:
- Unit profile: `Tests run: 440, Failures: 0, Errors: 0, Skipped: 3`
- Note: Test count is lower than baseline (504) due to removal of monitoring-specific tests
...
- `card-fraud-rule-engine-monitoring` (sibling MONITORING service)
...
- Wrong path casing (`/AUTH`) -> use lowercase path
...
- Redis operations hang -> all Redis ops now have 5s bounded timeouts
